...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.inviteEmail) {
      const invited = await createPortalResource("users", { customerId: client.id, name: body.inviteName || body.contactName || body.inviteEmail, email: body.inviteEmail, active: true }, user.id) as { generatedLink?: string };
      generatedLink = invited.generatedLink;
    }
    return NextResponse.json({ clientId: client.id, programmeId: programme.id, projectId: project.id, siteId: site.id, generatedLink, payload: await portalPayload("clients") });
  } catch (error) {
