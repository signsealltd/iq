...
export default async function ProjectPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-5 overflow-x-hidden xl:grid-cols-[minmax(0,1fr)_360px]">
      <section className="grid min-w-0 gap-5">
        <div>
          ...
          <p className="mt-2 max-w-3xl break-words text-steel">{project.description ?? "Project details are being prepared."}</p>
        ...
        <section className="panel p-4">
          ...
          <div className="mt-4 grid min-w-0 gap-2 md:grid-cols-3">
            {project.stages.filter((stage) => stage.enabled).map((stage) => <div key={stage.id} className="min-w-0 rounded-md border border-line bg-elevated p-3"><p className="font-semibold">{stage.label}</p><p className="text-sm text-steel">{stage.completedAt ? `Completed ${stage.completedAt.toLocaleDateString("en-GB")}` : "In progress / pending"}</p></div>)}
          </div>
        </section>
        ...
        <section className="panel p-4">
          ...
          <div className="mt-3 grid min-w-0 gap-3 md:grid-cols-2">
            {project.sites.map((site) => <article key={site.id} className="min-w-0 rounded-md border border-line bg-elevated p-3">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-semibold">{site.name}</p><p className="text-sm text-steel">{site.address ?? "Address pending"}</p></div><span className="max-w-[45%] shrink-0 break-words text-right text-xs text-steel">{formatPortalStatus(site.status)}</span></div>
        ...
        <section className="panel p-4">
          ...
          <div className="mt-3 grid gap-3">
            {project.messages.map((message) => <article key={message.id} className="min-w-0 rounded-md border border-line bg-elevated p-3"><div className="flex flex-wrap justify-between gap-2 text-sm"><strong className="min-w-0 break-words">{message.senderName}</strong><span className="break-words text-steel">{message.createdAt.toLocaleString("en-GB")} - {formatPortalStatus(message.visibility)}</span></div><p className="mt-2 whitespace-pre-wrap break-words text-sm text-steel">{message.body}</p>{isInternal ? <form action={deletePortalMessage} className="mt-3"><input type="hidden" name="projectId" value={project.id} /><input type="hidden" name="messageId" value={message.id} /><ConfirmSubmitButton className="button-secondary min-h-9 px-3 text-danger" message="Delete this portal message permanently?">Delete message</ConfirmSubmitButton></form> : null}</article>)}
