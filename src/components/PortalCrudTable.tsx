...
import { Archive, Copy, Edit3, Plus, RotateCcw, Search, Trash2, X } from "lucide-react";
...
type Payload = { rows: Row[]; lookups: Record<string, LookupOption[]>; generatedLink?: string };
...
export function PortalCrudTable({ resource, initialRows, lookups }: { resource: PortalResource; initialRows: Row[]; lookups: Record<string, LookupOption[]> }) {
  ...
  async function mutate(body: Record<string, unknown>) {
    ...
    setGeneratedLink(typeof data.generatedLink === "string" ? data.generatedLink : "");
    setRows(data.rows);
    setEditing(null);
  }
  ...
  return (
    <section className="grid gap-3">
      ...
      {generatedLink ? <div className="panel flex flex-col gap-2 p-3 text-sm md:flex-row md:items-center"><div className="mr-auto"><p className="font-semibold">Manual portal invite link generated</p><p className="break-all text-steel">{generatedLink}</p></div><button className="button" onClick={() => navigator.clipboard.writeText(generatedLink)}><Copy size={16} /> Copy link</button></div> : null}
